...
public class MarioAgentWithSound : Agent
{
    ...
    private TimeManagerWithSound timeManager;
    ...
    private void ResetAllObjects(GameObject currGameObject, string tagName)
    {
        ...
        {
            if (tagName == "mysteryBlock") { child.GetComponent<MysteryBlockManagerWithSound>().ResetMysteryBlock(); }
            if (tagName == "brickBlock") { child.GetComponent<BrickBlockManagerWithSound>().ResetBrickBlock(); }
            if (tagName == "goomba") { child.GetComponent<GoombaManagerWithSound>().ResetGoomba(); }
        }
    }
    ...
    IEnumerator CheckValidHit(Collision2D collision)
    {
        ...
        if (collision != null)
        {
            GoombaManagerWithSound hit = collision.gameObject.GetComponent<GoombaManagerWithSound>();
